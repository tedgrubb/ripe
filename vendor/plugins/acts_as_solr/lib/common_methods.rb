module ActsAsSolr #:nodoc:
  ...
  module CommonMethods
    ...
    def solr_add(add_xml)
      ActsAsSolr::Post.execute(Solr::Request::AddDocument.new(add_xml), :timeout => 60)
    end
    
    # Sends the delete command to Solr
    def solr_delete(solr_ids)
      ActsAsSolr::Post.execute(Solr::Request::Delete.new(:id => solr_ids), :timeout => 60)
    end
    
    # Sends the commit command to Solr
    def solr_commit
      ActsAsSolr::Post.execute(Solr::Request::Commit.new, :timeout => 120)
    end
    ...
    # 
    def solr_optimize
      ActsAsSolr::Post.execute(Solr::Request::Optimize.new, :timeout => 120)
    end
