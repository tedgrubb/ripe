...
module ActsAsSolr #:nodoc:
  ...
  module ClassMethods
    ...
    def kill_solr_index
      logger.debug "Killing solr index for #{self.name}"
      ActsAsSolr::Post.execute(Solr::Request::Delete.new(:query => "type_s:#{self.name}"))
      solr_commit
    end
    ...
    def clean_solr(q)
      ...
      clean_query = clean_query.gsub("\n", " ")
      clean_query = clean_query.gsub(/^(AND|OR|NOT)/, "")
      clean_query = clean_query.gsub(/(AND|OR|NOT)$/, "")
    end
