module ActsAsSolr #:nodoc:
  ...
  module InstanceMethods
    ...
    def validate_boost(boost)
      value = case boost
              when Symbol: self.send(boost)
              when Proc: boost.call(self)
              when Float: boost
              else
                boost.to_s.to_f
              end
      ...
      if value < 0.0
        logger.warn "The boost value has to be a float and posisive, but got #{value}. Using default boost value."
        value = solr_configuration[:default_boost]
      end
